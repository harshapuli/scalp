...
def incremental_update(df: pd.DataFrame, include_whale: bool = True) -> pd.DataFrame:
    ...
    # ADX: use the EWM approximation for last bar
    if 'ADX_14' in df.columns and n > 15:
        # Keep previous ADX value — full recompute is not needed per bar
        # ADX changes slowly (EWM of DX), previous value is close enough
        pass
