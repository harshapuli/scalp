...
def generate_demo_data():
    ...
    for i in range(80):
        ...
        # Whale-specific fields (70% of trades have whale confirmation)
        whale_confirmed = np.random.random() < 0.70
        whale_direction = np.random.choice(['LONG', 'SHORT', 'NEUTRAL'], p=[0.5, 0.35, 0.15]) if whale_confirmed else None
        whale_confidence = np.random.randint(60, 100) if whale_confirmed else 0
        whale_alert_type = np.random.choice(['ENTRY', 'TRAP', 'DIVERGENCE', 'EXHAUSTION', None], p=[0.4, 0.3, 0.15, 0.1, 0.05]) if whale_confirmed else None
        at_accumulation_zone = np.random.random() < 0.55 if whale_confirmed else False
        ...
        trade = {
            ...
            'whale_alert_type': whale_alert_type,
            'at_accumulation_zone': at_accumulation_zone,
        }
        trades.append(trade)
...
def generate_dashboard_html(session_data: dict, analysis_report: dict = None) -> str:
    ...
    # Generate whale intelligence stats
    whale_stats = {
        'whale_confirmed_stats': {},
        'non_whale_stats': {},
        'whale_alert_breakdown': {},
        'whale_direction_accuracy': {},
        'accumulation_zone_stats': {},
        'whale_trap_stats': {}
    }

    # Calculate whale stats from trades
    whale_trades = [t for t in trades if t.get('whale_confirmed')]
    non_whale_trades = [t for t in trades if not t.get('whale_confirmed')]

    if whale_trades:
        whale_wins = sum(1 for t in whale_trades if t.get('pnl_pct', 0) > 0)
        whale_stats['whale_confirmed_stats'] = {
            'count': len(whale_trades),
            'win_rate': (whale_wins / len(whale_trades) * 100) if whale_trades else 0,
            'total_pnl': sum(t.get('pnl_pct', 0) for t in whale_trades),
            'avg_pnl': sum(t.get('pnl_pct', 0) for t in whale_trades) / len(whale_trades) if whale_trades else 0
        }

    if non_whale_trades:
        non_wins = sum(1 for t in non_whale_trades if t.get('pnl_pct', 0) > 0)
        whale_stats['non_whale_stats'] = {
            'count': len(non_whale_trades),
            'win_rate': (non_wins / len(non_whale_trades) * 100) if non_whale_trades else 0,
            'total_pnl': sum(t.get('pnl_pct', 0) for t in non_whale_trades),
            'avg_pnl': sum(t.get('pnl_pct', 0) for t in non_whale_trades) / len(non_whale_trades) if non_whale_trades else 0
        }

    # Alert breakdown
    for trade in whale_trades:
        alert_type = trade.get('whale_alert_type', 'NONE')
        if alert_type not in whale_stats['whale_alert_breakdown']:
            whale_stats['whale_alert_breakdown'][alert_type] = {'count': 0, 'wins': 0}
        whale_stats['whale_alert_breakdown'][alert_type]['count'] += 1
        if trade.get('pnl_pct', 0) > 0:
            whale_stats['whale_alert_breakdown'][alert_type]['wins'] += 1

    # Direction accuracy
    long_trades = [t for t in whale_trades if t.get('whale_direction') == 'LONG']
    short_trades = [t for t in whale_trades if t.get('whale_direction') == 'SHORT']

    if long_trades:
        long_wins = sum(1 for t in long_trades if t.get('pnl_pct', 0) > 0)
        whale_stats['whale_direction_accuracy']['LONG'] = {
    ...
    if short_trades:
        ...
        whale_stats['whale_direction_accuracy']['SHORT'] = {
            'count': len(short_trades),
            'win_rate': (short_wins / len(short_trades) * 100) if short_trades else 0
        }

    # Accumulation zone stats
    at_zone = [t for t in whale_trades if t.get('at_accumulation_zone')]
    not_at_zone = [t for t in whale_trades if not t.get('at_accumulation_zone')]

    if at_zone:
        zone_wins = sum(1 for t in at_zone if t.get('pnl_pct', 0) > 0)
        whale_stats['accumulation_zone_stats']['at_zone'] = {
            'count': len(at_zone),
            'win_rate': (zone_wins / len(at_zone) * 100) if at_zone else 0
        }

    if not_at_zone:
        no_zone_wins = sum(1 for t in not_at_zone if t.get('pnl_pct', 0) > 0)
        whale_stats['accumulation_zone_stats']['not_at_zone'] = {
            'count': len(not_at_zone),
            'win_rate': (no_zone_wins / len(not_at_zone) * 100) if not_at_zone else 0
        }

    # Whale trap performance
    trap_trades = [t for t in whale_trades if t.get('whale_alert_type') == 'TRAP']
    if trap_trades:
        trap_wins = sum(1 for t in trap_trades if t.get('pnl_pct', 0) > 0)
        whale_stats['whale_trap_stats'] = {
            'count': len(trap_trades),
            'win_rate': (trap_wins / len(trap_trades) * 100) if trap_trades else 0,
            'avg_pnl': sum(t.get('pnl_pct', 0) for t in trap_trades) / len(trap_trades) if trap_trades else 0
        }

    whale_json = json.dumps(whale_stats, default=str)
...
<div class="tabs">
    ...
    <div class="tab" onclick="switchTab('whale')">Whale Intelligence</div>
</div>
...
<!-- WHALE INTELLIGENCE TAB -->
<div id="tab-whale" class="tab-content">
    <div class="kpi-grid">
        <div class="kpi"><div class="label">Net Whale Flow</div><div class="value" id="whaleNetFlow">--</div><div class="label" style="font-size: 11px; color: var(--text-dim); margin-top: 4px;">Cumulative Long vs Short</div></div>
        <div class="kpi"><div class="label">Whale Trade Win Rate</div><div class="value" id="whaleWinRate">--</div><div class="label" style="font-size: 11px; color: var(--text-dim); margin-top: 4px;">Whale Confirmed Trades</div></div>
        <div class="kpi"><div class="label">Avg Whale Conviction</div><div class="value" id="avgWhaleConviction">--</div><div class="label" style="font-size: 11px; color: var(--text-dim); margin-top: 4px;">0-100 Score</div></div>
        <div class="kpi"><div class="label">Whale Signal Count</div><div class="value" id="whaleSignalCount">--</div><div class="label" style="font-size: 11px; color: var(--text-dim); margin-top: 4px;">Total Trades Used</div></div>
    </div>

    <div class="charts-grid">
        <div class="chart-card"><h3>Whale vs Non-Whale P&L</h3><canvas id="whalePnlChart"></canvas></div>
        <div class="chart-card"><h3>Whale Alert Distribution</h3><canvas id="whaleAlertChart"></canvas></div>
        <div class="chart-card"><h3>Whale Direction Accuracy</h3><canvas id="whaleAccuracyChart"></canvas></div>
        <div class="chart-card"><h3>Accumulation Zone Hit Rate</h3><canvas id="accumZoneChart"></canvas></div>
        <div class="chart-card"><h3>Whale Trap Performance</h3><canvas id="trapPerfChart"></canvas></div>
        <div class="chart-card"><h3>Whale Confidence Distribution</h3><canvas id="whaleConfidenceChart"></canvas></div>
    </div>

    <div class="table-card">
        <h3>Whale-Confirmed Trades Analysis</h3>
        <table id="whaleTable">
            <thead><tr>
                <th onclick="sortTable('whaleTable',0)">ID</th>
                <th onclick="sortTable('whaleTable',1)">Alert Type</th>
                <th onclick="sortTable('whaleTable',2)">Direction</th>
                <th onclick="sortTable('whaleTable',3)">Confidence</th>
                <th onclick="sortTable('whaleTable',4)">At Accum Zone</th>
                <th onclick="sortTable('whaleTable',5)">Entry</th>
                <th onclick="sortTable('whaleTable',6)">Exit</th>
                <th onclick="sortTable('whaleTable',7)">P&L %</th>
                <th onclick="sortTable('whaleTable',8)">Outcome</th>
            </tr></thead>
...
function renderWhaleIntelligence() {{
    // Filter trades: whale-confirmed vs non-whale
    const whaleTrades = ALL_TRADES.filter(t => t.whale_confirmed);
    const nonWhaleTrades = ALL_TRADES.filter(t => !t.whale_confirmed);

    // Calculate whale KPIs
    const whaleWins = whaleTrades.filter(t => t.pnl_pct > 0).length;
    const whaleWinRate = whaleTrades.length > 0 ? (whaleWins / whaleTrades.length * 100).toFixed(1) : 0;
    const whaleNetFlow = whaleTrades.reduce((sum, t) => {{
        const longFlow = (t.whale_direction === 'LONG' ? 1 : 0) * (t.pnl_pct > 0 ? 1 : -1);
        return sum + longFlow;
    }}, 0);
    const avgWhaleConf = whaleTrades.length > 0 ? (whaleTrades.reduce((sum, t) => sum + (t.whale_confidence || 0), 0) / whaleTrades.length).toFixed(0) : 0;

    document.getElementById('whaleNetFlow').textContent = whaleNetFlow >= 0 ? '+' + whaleNetFlow : whaleNetFlow;
    document.getElementById('whaleWinRate').textContent = whaleWinRate + '%';
    document.getElementById('avgWhaleConviction').textContent = avgWhaleConf;
    document.getElementById('whaleSignalCount').textContent = whaleTrades.length;

    // Whale vs Non-Whale P&L comparison
    const whaleAvgPnl = whaleTrades.length > 0 ? (whaleTrades.reduce((sum, t) => sum + t.pnl_pct, 0) / whaleTrades.length).toFixed(2) : 0;
    const nonWhaleAvgPnl = nonWhaleTrades.length > 0 ? (nonWhaleTrades.reduce((sum, t) => sum + t.pnl_pct, 0) / nonWhaleTrades.length).toFixed(2) : 0;
    const whaleTotalPnl = whaleTrades.reduce((sum, t) => sum + t.pnl_pct, 0).toFixed(2);
    const nonWhaleTotalPnl = nonWhaleTrades.reduce((sum, t) => sum + t.pnl_pct, 0).toFixed(2);

    charts.whalePnl = new Chart(document.getElementById('whalePnlChart'), {{
        type: 'bar',
        data: {{
            labels: ['Avg P&L %', 'Total P&L %'],
            datasets: [
                {{ label: 'Whale Confirmed', data: [whaleAvgPnl, whaleTotalPnl], backgroundColor: '#6366f188', borderColor: '#6366f1', borderWidth: 1 }},
                {{ label: 'Non-Whale', data: [nonWhaleAvgPnl, nonWhaleTotalPnl], backgroundColor: '#ef444488', borderColor: '#ef4444', borderWidth: 1 }},
            ]
        }},
        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ labels: {{ color: '#8b8d97' }} }} }}, scales: {{ x: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }} }}, y: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }} }} }} }}
    }});

    // Whale alert distribution
    const alertCounts = {{}};
    whaleTrades.forEach(t => {{
        const type = t.whale_alert_type || 'NONE';
        alertCounts[type] = (alertCounts[type] || 0) + 1;
    }});

    const alertLabels = Object.keys(alertCounts);
    const alertData = Object.values(alertCounts);
    const colors = ['#6366f1', '#22c55e', '#f97316', '#eab308', '#ef4444'];

    charts.whaleAlert = new Chart(document.getElementById('whaleAlertChart'), {{
        type: 'doughnut',
        data: {{
            labels: alertLabels,
            datasets: [{{
                data: alertData,
                backgroundColor: colors.slice(0, alertLabels.length),
                borderColor: colors.slice(0, alertLabels.length),
                borderWidth: 2
            }}]
        }},
        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ labels: {{ color: '#8b8d97' }}, position: 'bottom' }} }} }}
    }});

    // Whale direction accuracy (when whale said BUY, was trade profitable?)
    const bullishTrades = whaleTrades.filter(t => t.whale_direction === 'LONG');
    const bearishTrades = whaleTrades.filter(t => t.whale_direction === 'SHORT');
    const bullishWins = bullishTrades.filter(t => t.pnl_pct > 0).length;
    const bearishWins = bearishTrades.filter(t => t.pnl_pct > 0).length;
    const bullishAccuracy = bullishTrades.length > 0 ? (bullishWins / bullishTrades.length * 100).toFixed(0) : 0;
    const bearishAccuracy = bearishTrades.length > 0 ? (bearishWins / bearishTrades.length * 100).toFixed(0) : 0;

    charts.whaleAccuracy = new Chart(document.getElementById('whaleAccuracyChart'), {{
        type: 'bar',
        data: {{
            labels: ['LONG Direction', 'SHORT Direction'],
            datasets: [{{
                label: 'Win Rate %',
                data: [bullishAccuracy, bearishAccuracy],
                backgroundColor: ['#22c55e88', '#ef444488'],
                borderColor: ['#22c55e', '#ef4444'],
                borderWidth: 1
            }}]
        }},
        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ display: false }} }}, scales: {{ x: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }} }}, y: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }}, max: 100 }} }} }}
    }});

    // Accumulation zone hit rate
    const atZoneTrades = whaleTrades.filter(t => t.at_accumulation_zone);
    const notAtZoneTrades = whaleTrades.filter(t => !t.at_accumulation_zone);
    const zoneWins = atZoneTrades.filter(t => t.pnl_pct > 0).length;
    const noZoneWins = notAtZoneTrades.filter(t => t.pnl_pct > 0).length;
    const zoneWinRate = atZoneTrades.length > 0 ? (zoneWins / atZoneTrades.length * 100).toFixed(0) : 0;
    const noZoneWinRate = notAtZoneTrades.length > 0 ? (noZoneWins / notAtZoneTrades.length * 100).toFixed(0) : 0;

    charts.accumZone = new Chart(document.getElementById('accumZoneChart'), {{
        type: 'bar',
        data: {{
            labels: ['At Accum Zone', 'Outside Zone'],
            datasets: [{{
                label: 'Win Rate %',
                data: [zoneWinRate, noZoneWinRate],
                backgroundColor: ['#a855f788', '#8b8d9788'],
                borderColor: ['#a855f7', '#8b8d97'],
                borderWidth: 1
            }}]
        }},
        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ display: false }} }}, scales: {{ x: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }} }}, y: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }}, max: 100 }} }} }}
    ...
    // Whale trap performance (whale_alert_type === 'TRAP')
    const trapTrades = whaleTrades.filter(t => t.whale_alert_type === 'TRAP');
    const trapWins = trapTrades.filter(t => t.pnl_pct > 0).length;
    const trapAvgPnl = trapTrades.length > 0 ? (trapTrades.reduce((sum, t) => sum + t.pnl_pct, 0) / trapTrades.length).toFixed(2) : 0;
    const trapWinRate = trapTrades.length > 0 ? (trapWins / trapTrades.length * 100).toFixed(0) : 0;

    charts.trapPerf = new Chart(document.getElementById('trapPerfChart'), {{
        type: 'bar',
        data: {{
            labels: ['Win Rate %', 'Avg P&L %'],
            datasets: [{{
                label: 'Whale Trap Reversal',
                data: [trapWinRate, trapAvgPnl],
                backgroundColor: ['#f97316', '#eab308'],
                borderColor: ['#f97316', '#eab308'],
                borderWidth: 1
            }}]
        }},
        options: {{ responsive: true, maintainAspectRatio: false, plugins: {{ legend: {{ display: false }} }}, scales: {{ x: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }} }}, y: {{ ticks: {{ color: '#8b8d97' }}, grid: {{ color: '#2a2d3a' }} }} }} }}
    ...
    const confBuckets = {{ '0-40': 0, '40-60': 0, '60-80': 0, '80-100': 0 }};
    whaleTrades.forEach(t => {{
        if (t.whale_confidence < 40) confBuckets['0-40']++;
        else if (t.whale_confidence < 60) confBuckets['40-60']++;
        else if (t.whale_confidence < 80) confBuckets['60-80']++;
        else confBuckets['80-100']++;
    }});

    charts.whaleConf = new Chart(document.getElementById('whaleConfidenceChart'), {{
        type: 'bar',
        data: {{
    ...
    // Whale trades table
    document.getElementById('whaleBody').innerHTML = whaleTrades.map(t => `
        <tr>
            <td>${{t.trade_id}}</td>
            <td>${{t.whale_alert_type || '—'}}</td>
            <td style="color:${{t.whale_direction === 'LONG' ? 'var(--green)' : t.whale_direction === 'SHORT' ? 'var(--red)' : 'var(--text-dim)'}};">${{t.whale_direction || '—'}}</td>
            <td>${{t.whale_confidence || 0}}</td>
            <td style="color:${{t.at_accumulation_zone ? 'var(--green)' : 'var(--text-dim)'}}">${{t.at_accumulation_zone ? 'Yes' : 'No'}}</td>
            <td>${{t.entry_price}}</td>
            <td>${{t.exit_price}}</td>
            <td style="color:${{t.pnl_pct > 0 ? 'var(--green)' : 'var(--red)'}}">${{t.pnl_pct > 0 ? '+' : ''}}${{t.pnl_pct}}%</td>
            <td style="color:${{t.pnl_pct > 0 ? 'var(--green)' : 'var(--red)'}}">${{t.outcome}}</td>
        </tr>
    `).join('') || '<tr><td colspan="9" style="text-align:center; color:var(--text-dim)">No whale-confirmed trades</td></tr>';
}}
...
function switchTab(name) {{
    ...
    if (name === 'whale') renderWhaleIntelligence();
}}
