    ...
    try:
        ...
        if signal_func == 'smc_order_block':
            ...
            if has_smc and 'SM_Divergence_Bull' in df.columns:
                ...
                rsi = df['RSI_14'].values if 'RSI_14' in df.columns else np.full(n, 50)
                c = df['Close'].values
                c_prev10 = pd.Series(c).shift(10).values
                rsi_prev10 = pd.Series(rsi).shift(10).values
                
                signal_call = (c < c_prev10) & (rsi > rsi_prev10) & (rsi < 40)
                signal_put = (c > c_prev10) & (rsi < rsi_prev10) & (rsi > 60)
                
                signal_call[:20] = False
                signal_put[:20] = False

        elif signal_func == 'turtle_soup':
            # Failed 20-bar breakout reversal
            h, l, c = df['High'].values, df['Low'].values, df['Close'].values
            rh_prev = pd.Series(h).rolling(20).max().shift(1).values
            rl_prev = pd.Series(l).rolling(20).min().shift(1).values
            
            signal_put = (h > rh_prev) & (c < rh_prev)
            signal_call = (l < rl_prev) & (c > rl_prev)
            
            signal_call[:21] = False
            signal_put[:21] = False
        ...
        # ══════════════════════════════════════════════════════════
            ...
            c = df['Close'].values
            vwap = df['VWAP'].values if 'VWAP' in df.columns else c
            ema9_prev = pd.Series(ema9).shift(1).values
            ema21_prev = pd.Series(ema21).shift(1).values
            
            signal_call = (ema9_prev <= ema21_prev) & (ema9 > ema21) & (c > vwap)
            signal_put  = (ema9_prev >= ema21_prev) & (ema9 < ema21) & (c < vwap)
            ...
            near_vwap_prev = pd.Series(near_vwap).shift(1).values
            vol_ratio = df['Volume'].values / (df['Volume_MA_20'].values + 1e-10) if 'Volume_MA_20' in df.columns else np.ones(n)
            vol_ok = vol_ratio > 1.2
            
            signal_call = near_vwap_prev & (c > vwap) & vol_ok
            signal_put  = near_vwap_prev & (c < vwap) & vol_ok
        ...
        # ══════════════════════════════════════════════════════════
            ...
            if has_smc and 'Bull_Sweep' in df.columns and 'CHoCH_Bull' in df.columns:
                ...
                rl_prev = pd.Series(l).rolling(20).min().shift(1).values
                rh_prev = pd.Series(h).rolling(20).max().shift(1).values
                c_prev = pd.Series(c).shift(1).values
                
                signal_call = (l < rl_prev) & (c > c_prev)
                signal_put  = (h > rh_prev) & (c < c_prev)
            ...
            if has_smc and 'Bull_Sweep' in df.columns and 'Displacement_Bull' in df.columns:
                ...
                signal_call = (l < rl_prev) & (body > atr * 1.5) & (c > o)
                signal_put  = (h > rh_prev) & (body > atr * 1.5) & (c < o)
            ...
            if has_smc and 'In_OTE_Long' in df.columns and 'Bull_FVG' in df.columns:
                ...
                signal_call = (rsi < 40) & (c > sma)
                signal_put  = (rsi > 60) & (c < sma)
                
                signal_call[:50] = False
                signal_put[:50] = False
        ...
        # ══════════════════════════════════════════════════════════
            ...
            signal_call = (ema9_prev <= ema21_prev) & (ema9 > ema21)
            signal_put  = (ema9_prev >= ema21_prev) & (ema9 < ema21)
...
def apply_v2_filters(
    ...
    if v2_config is None:
        try:
            from config_v2 import V2_FILTERS
        except ImportError:
...
def _fallback_sweep_reversal(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    ...
    h, l, c = df['High'].values, df['Low'].values, df['Close'].values
    rh_prev2 = pd.Series(h).rolling(20).max().shift(2).values
    rl_prev2 = pd.Series(l).rolling(20).min().shift(2).values
    h_prev = pd.Series(h).shift(1).values
    l_prev = pd.Series(l).shift(1).values
    c_prev = pd.Series(c).shift(1).values
    
    sp = (h_prev > rh_prev2) & (c < c_prev)
    sc = (l_prev < rl_prev2) & (c > c_prev)
    
    sp[:21] = False
    sc[:21] = False
    return sc.astype(bool), sp.astype(bool)
